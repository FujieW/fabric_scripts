  ...
  - Name: ${ORG_NAME}
    ...
    Template:
      ...
      SANS:
        - localhost
    Users:
      Count: ${USER_COUNT}" > ./${ORG_NAME}-crypto.yaml
}
...
function generateOrg() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo
    echo "########### generate the crypto yaml for this org ########"
    set -x
    generateCryptoYaml
    cat ./${ORG_NAME}-crypto.yaml
    set +x
    sleep 2
...
#  fi
  ...
  if [ ! -d "../organizations/peerOrganizations/${DOMAIN_NAME}" ]; then
    generateOrg
